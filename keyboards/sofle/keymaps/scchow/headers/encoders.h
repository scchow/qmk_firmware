...
void alt_tab(bool clockwise){
    ...
    if (clockwise) {
        ...
        // If detent was negative, 
        // previously the encoder was turned the opposite direction
        // reset counter back to 0
        if (detents < 0){
            detents = 0;
        }

        // Increment the detent counter
        detents += 1;

        // If we have turned the encoder enough, send a tab
        if (detents >= DETENTS_PER_TAB){
            tap_code16(KC_TAB);
            detents = 0;
        }
        ...
        // Decrement the detent counter
        detents -= 1;

        // If we have turned the encoder enough, send a shift tab
        if (detents <= -DETENTS_PER_TAB){
            tap_code16(S(KC_TAB));
            detents = 0;
        }
